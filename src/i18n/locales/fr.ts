import type { Messages } from './en';

const fr: Messages = {
  common: {
    appName: 'Warranty Store',
    cancel: 'Annuler',
    delete: 'Supprimer',
    close: 'Fermer',
    systemDefault: 'Système par défaut',
    initializingDatabase: 'Initialisation de la base de données…',
    databaseError: 'Erreur de base de données : %{error}',

    retry: 'Réessayer',
    pageOfTotal: 'Page %{current} sur %{total}',
  },
  nav: {
    home: 'Accueil',
    products: 'Produits',
    settings: 'Paramètres',
    goBack: 'Retour',
    addItem: 'Ajouter un produit',
  },
  category: {
    electronics: 'Électronique',
    appliances: 'Électroménager',
    furniture: 'Meubles',
    vehicles: 'Véhicules',
    other: 'Autre',
    uncategorized: 'Sans catégorie',
    allCategories: 'Toutes les catégories',
    filterByCategory: 'Filtrer par catégorie',
    selectCategory: 'Sélectionner une catégorie',
  },
  status: {
    all: 'Tous',
    active: 'Active',
    expiringSoon: 'Expire bientôt',
    expired: 'Expirée',
    upcoming: 'À venir',
  },
  home: {
    search: 'Rechercher',
    searchPlaceholder: 'Rechercher des produits...',
    overview: 'Aperçu',
    emptyTitle: "Aucun produit pour l'instant.",
    emptySubtitle: 'Ajoutez votre premier produit pour suivre sa garantie.',
    addProduct: 'Ajouter un produit',
    expiredOn: 'Expiré le %{date}',
    expiresOn: 'Expire le %{date}',
  },
  warrantyStatus: {
    title: 'État des garanties',
    expiredHeadline: {
      one: '%{count} garantie a expiré',
      other: '%{count} garanties ont expiré',
    },
    expiringHeadline: {
      one: '%{count} garantie expire bientôt',
      other: '%{count} garanties expirent bientôt',
    },
    expiredSegment: { one: '%{count} expirée', other: '%{count} expirées' },
    expiringSegment: { one: '%{count} expire bientôt', other: '%{count} expirent bientôt' },
    expiredSubtitle: 'Certaines garanties nécessitent peut-être votre attention.',
    attentionSubtitle: 'Vous avez des garanties qui nécessitent peut-être votre attention.',
    caughtUpTitle: 'Tout est à jour !',
    caughtUpSubtitle: "Aucune garantie expirée ou sur le point d'expirer.",
    viewAllExpired: {
      one: 'Voir %{count} garantie expirée',
      other: 'Voir les %{count} expirées',
    },
    viewAllExpiring: {
      one: 'Voir %{count} garantie qui expire bientôt',
      other: 'Voir les %{count} qui expirent bientôt',
    },
    reviewAll: { one: 'Examiner %{count} garantie', other: 'Examiner les %{count} garanties' },
  },
  recentProducts: {
    title: 'Produits récents',
    seeAll: 'Tout voir',
    seeAllHint: 'Voir tous les produits',
  },
  products: {
    title: 'Mes produits',
    subtitle: 'Tous vos produits au même endroit',
    count: { one: '%{count} produit', other: '%{count} produits' },
    emptyTitle: "Aucun produit pour l'instant.",
    emptySubtitle: 'Ajoutez votre premier produit pour suivre sa garantie.',
    noResultsTitle: 'Aucun produit ne correspond à vos filtres.',
    noResultsSubtitle: 'Essayez une autre recherche, un autre statut ou une autre catégorie.',
  },
  sort: {
    title: 'Trier par',
    newest: "Plus récents d'abord",
    oldest: "Plus anciens d'abord",
    nameAsc: 'A–Z',
    priceDesc: 'Prix (du plus élevé au plus bas)',
    expirySoonest: 'Expiration de garantie (la plus proche)',
    expiryLatest: 'Expiration de garantie (la plus lointaine)',
  },
  itemDetail: {
    title: 'Détail du produit',
    notFound: 'Produit introuvable.',
    deleteConfirmTitle: 'Supprimer le produit ?',
    deleteConfirmMessage: '« %{name} » et ses détails seront définitivement supprimés.',
    itemDeleted: 'Produit supprimé',
    deleteFailed: 'Impossible de supprimer le produit. Veuillez réessayer.',
    brand: 'Marque',
    purchaseDate: "Date d'achat",
    purchasePrice: "Prix d'achat",
    store: 'Magasin',
    warrantyPeriod: 'Durée de garantie',
    warrantyValidTill: "Garantie valable jusqu'au",
    notes: 'Notes',
    noNotes: 'Aucune note ajoutée.',
    invoiceDocuments: 'Facture originale (Reçu / Facture)',
    warrantyDocuments: "Garantie d'origine (Fabricant)",
    viewDocumentHint: 'Appuyez pour voir en plein écran',
    editItem: 'Modifier le produit',
    deleteItem: 'Supprimer le produit',
    deleting: 'Suppression…',
    category: 'Catégorie',
    coveredTill: 'Couvert jusqu’au %{date}',
    daysLeft: { one: '%{count} jour restant', other: '%{count} jours restants' },
    originalWarrantyValidTill: 'Garantie d’origine valable jusqu’au',
    extendedWarrantyValidTill: 'Garantie prolongée valable jusqu’au',
    originalBills: 'Factures d’origine',
    originalBillsSubtitle: 'Factures ou reçus de l’achat du produit',
    warrantyCoverage: 'Couverture de garantie',
    addExtendedWarranty: 'Ajouter une garantie prolongée',
    originalWarranty: 'Garantie d’origine (fabricant)',
    extendedWarranty: 'Garantie prolongée',
    warrantyDocumentsWithExamples:
      'Documents de garantie (facture, reçu, carte, certificat, etc.)',
    extendedInvoiceSection: 'Facture de la garantie prolongée',
    extendedDocumentsSection:
      'Documents de garantie (certificat, police, conditions generales, etc.)',
    provider: 'Fournisseur',
    duration: 'Durée',
    cost: 'Coût',
    addMore: 'Ajouter',
    addInvoice: 'Ajouter une facture',
    documentMenu: 'Options du document',
    viewDocument: 'Afficher',
    editInItem: 'Modifier dans le produit',
    noDocuments: 'Aucun document pour l’instant',
  },
  addEditItem: {
    editTitle: 'Modifier le produit',
    addTitle: 'Ajouter un produit',
    notFound: 'Produit introuvable.',
    cameraPermissionTitle: 'Autorisation de la caméra requise',
    cameraPermissionMessage:
      "Warranty Store a besoin d'accéder à la caméra pour photographier votre facture. Vous pouvez l'activer dans les Paramètres.",
    galleryPermissionTitle: 'Autorisation de la photothèque requise',
    galleryPermissionMessage:
      "Warranty Store a besoin d'accéder à votre photothèque pour joindre une photo de votre facture. Vous pouvez l'activer dans les Paramètres.",
    openSettings: 'Ouvrir les paramètres',
    notificationPermissionTitle: 'Restez informé de vos garanties',
    notificationPermissionMessage:
      'Nous vous rappellerons avant l\'expiration de vos garanties. Autoriser les notifications ?',
    notificationPermissionAllow: 'Autoriser les notifications',
    photoSaveFailed: "Impossible d'enregistrer la photo du produit. Veuillez réessayer.",
    itemUpdated: 'Produit mis à jour',
    itemAdded: 'Produit ajouté',
    nameRequired: 'Le nom est requis',
    warrantyMonthsInvalid: 'Indiquez la durée de garantie en mois entiers (ex. 12)',
    saveFailed: "Impossible d'enregistrer le produit. Veuillez réessayer.",
    changePhoto: 'Changer la photo',
    addPhoto: 'Ajouter une photo',
    tapToUpload: 'Touchez pour importer',
    photoSourceTitle: 'Photo du produit',
    removePhoto: 'Supprimer la photo',
    requiredLegend: '* Champ obligatoire',
    requiredField: 'Obligatoire',
    itemName: 'Nom du produit',
    itemNamePlaceholder: 'p. ex. Dell XPS 13 Laptop',
    category: 'Catégorie',
    brand: 'Marque',
    brandPlaceholder: 'Entrez la marque',
    purchaseDate: "Date d'achat",
    selectDate: 'Sélectionner une date',
    purchasePrice: "Prix d'achat",
    enterAmount: 'Entrez le montant',
    store: 'Magasin',
    storePlaceholder: 'Entrez le nom du magasin',
    invoiceSection: 'Facture originale (Reçu / Facture)',
    invoiceSectionSubtitle: "Facture ou reçu de l'achat du produit",
    addInvoice: 'Ajouter une facture',
    warrantySection: "Garantie d'origine (Fabricant)",
    warrantyDocumentsLabel: 'Documents de garantie',
    warrantyDocumentsSubtitle: 'Carte de garantie, certificat, conditions générales, etc.',
    addDocument: 'Ajouter un document',
    addMore: 'Ajouter plus',
    multipleImagesHint: 'Vous pouvez ajouter plusieurs images.',
    attachDocumentTitle: 'Joindre un document',
    replaceDocumentTitle: 'Remplacer le document',
    takePhoto: 'Prendre une photo',
    chooseFromGallery: 'Choisir dans la galerie',
    savingEllipsis: 'Enregistrement…',
    documentsAttached: { one: '%{count} document joint', other: '%{count} documents joints' },
    addPage: 'Ajouter un document',
    removePage: 'Supprimer le document',
    replacePage: 'Remplacer le document',
    movePageLeft: 'Déplacer le document vers la gauche',
    movePageRight: 'Déplacer le document vers la droite',
    documentSaveFailed: "Impossible d'enregistrer le document. Veuillez réessayer.",
    maxDocumentsReached: "Vous pouvez joindre jusqu'à %{max} documents par section.",
    warrantyValidTill: "Garantie valable jusqu'au",
    warrantyValidTillCaption: "Calculée à partir de la date d'achat",
    warrantyMonths: 'Garantie (mois)',
    warrantyMonthsPlaceholder: 'p. ex. 12',
    notes: 'Notes',
    notesPlaceholder: 'Notes facultatives',
    save: 'Enregistrer le produit',
    extendedWarrantyOptional: '(Facultatif)',
    extendedWarrantyEntry: 'Garantie prolongée %{index}',
    provider: 'Fournisseur / Société',
    providerPlaceholder: 'Saisissez le fournisseur ou la société',
    duration: 'Durée',
    durationPlaceholder: 'p. ex. 24',
    durationMonths: 'mois',
    durationYears: 'ans',
    startsOn: 'Commence le',
    endsOn: 'Se termine le',
    endsOnCaption: 'Calculé à partir de la date de début et de la durée',
    cost: 'Coût',
    extendedNotesPlaceholder: 'p. ex. Garantie prolongée achetée en ligne',
    extendedInvoiceSection: 'Facture de la garantie prolongée',
    extendedInvoiceSubtitle: 'Facture ou reçu de l’achat de la garantie prolongée',
    extendedDocumentsSection: 'Documents de la garantie prolongée',
    extendedDocumentsSubtitle: 'Certificat, police, conditions générales, etc.',
    addExtendedWarranty: 'Ajouter une garantie prolongée',
    addAnotherExtendedWarranty: 'Ajouter une autre garantie prolongée',
    removeExtendedWarranty: 'Supprimer la garantie prolongée',
    extendedWarrantyHeading: 'Garantie prolongée',
    removeExtendedWarrantyTitle: 'Supprimer la garantie prolongée ?',
    removeExtendedWarrantyMessage:
      'La garantie prolongée %{index} et ses documents seront supprimés à l’enregistrement.',
    durationInvalid: 'Saisissez la durée en nombre entier (p. ex. 24)',
    costInvalid: 'Saisissez un coût égal ou supérieur à zéro',
    extendedWarrantyFieldInvalid: 'Vérifiez la garantie prolongée %{index} : %{message}',
    howItWorksTitle: 'Comment ça marche',
    howItWorksBody:
      'Votre garantie prolongée commence à la fin de la garantie d’origine. Vous serez prévenu avant la fin de chaque période de garantie.',
  },
  settings: {
    title: 'Paramètres',
    appearance: 'Apparence',
    theme: 'Thème',
    themeLight: 'Clair',
    themeDark: 'Sombre',
    language: 'Langue',
    general: 'Général',
    exportData: 'Exporter les données',
    exportBackupNeverExported: 'Aucune sauvegarde effectuée',
    exportBackupLastExported: 'Dernière sauvegarde : %{date}',
    exportBackupSuccess: 'Sauvegarde exportée',
    exportBackupFailed: "Échec de l'exportation de la sauvegarde",
    exportBackupMissingFilesTitle: 'Certaines images sont introuvables',
    exportBackupMissingFilesMessage: {
      one: "1 image référencée par vos produits est introuvable sur cet appareil. Exporter maintenant l'exclura de la sauvegarde.",
      other:
        '%{count} images référencées par vos produits sont introuvables sur cet appareil. Exporter maintenant les exclura de la sauvegarde.',
    },
    exportBackupMissingFilesAction: 'Exporter quand même',
    exportBackupSuccessSkipped: {
      one: 'Sauvegarde exportée sans 1 image manquante',
      other: 'Sauvegarde exportée sans %{count} images manquantes',
    },
    importData: 'Importer une sauvegarde',
    importBackupSubtitle: 'Restaurer vos produits depuis un fichier de sauvegarde',
    importBackupConfirmTitle: 'Importer cette sauvegarde ?',
    importBackupConfirmMessage: {
      one: 'Cette sauvegarde contient 1 produit. Vos produits existants sont conservés : seuls les produits que vous n’avez pas encore seront ajoutés.',
      other:
        'Cette sauvegarde contient %{count} produits. Vos produits existants sont conservés : seuls les produits que vous n’avez pas encore seront ajoutés.',
    },
    importBackupConfirmAction: 'Importer',
    importBackupSuccess: { one: '1 produit importé', other: '%{count} produits importés' },
    importBackupSuccessSkipped: '%{imported} importés, %{skipped} déjà dans votre bibliothèque',
    importBackupNothingNew: 'Tout le contenu de cette sauvegarde est déjà dans votre bibliothèque',
    importBackupEmpty: 'Cette sauvegarde ne contient aucun produit à importer',
    importBackupInvalid: "Ce fichier n'est pas une sauvegarde Warranty Store valide",
    importBackupUnsupportedVersion: 'Cette sauvegarde a été créée avec une version plus récente de l\'app',
    importBackupFailed: "Échec de l'importation de la sauvegarde",
    notificationsEnabled: 'Notifications activées',
    about: 'À propos',
    aboutApp: 'À propos de Warranty Store',
    version: 'Version',
    build: 'Build',
  },
  date: {
    addedToday: 'Ajouté aujourd’hui',
    addedYesterday: 'Ajouté hier',
    addedDaysAgo: { one: 'Ajouté il y a %{count} jour', other: 'Ajouté il y a %{count} jours' },
    addedWeeksAgo: { one: 'Ajouté il y a %{count} semaine', other: 'Ajouté il y a %{count} semaines' },
    addedMonthsAgo: { one: 'Ajouté il y a %{count} mois', other: 'Ajouté il y a %{count} mois' },
    addedYearsAgo: { one: 'Ajouté il y a %{count} an', other: 'Ajouté il y a %{count} ans' },
    expiresToday: "Expire aujourd'hui",
    expiresIn: { one: 'Expire dans %{count} jour', other: 'Expire dans %{count} jours' },
    expiredAgo: { one: 'Expiré il y a %{count} jour', other: 'Expiré il y a %{count} jours' },
    startsIn: { one: 'Commence dans %{count} jour', other: 'Commence dans %{count} jours' },
  },
  notifications: {
    reminderTitle: 'Rappel de garantie : %{name}',
  },
  duration: {
    months: { one: '%{count} mois', other: '%{count} mois' },
    years: { one: '%{count} an', other: '%{count} ans' },
  },
};

export default fr;
