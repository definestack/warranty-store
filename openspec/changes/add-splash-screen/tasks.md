...
- [x] 1.1 Install the SDK-matched splash package with `npx expo install expo-splash-screen`
      and confirm `package.json` records it in `dependencies` (not `devDependencies`).
      GitHub: #77
- [x] 1.2 Add the `expo-splash-screen` plugin entry to `app.json` `expo.plugins` with
      ...
      GitHub: #77
- [x] 1.3 Add a comment in `src/theme/palette.ts` noting that the two splash background
      literals in `app.json` must stay equal to the MD3 light/dark `background` values.
      GitHub: #77
- [x] 1.4 Run `npx expo-doctor` and confirm it passes with the new plugin entry.
      GitHub: #77

## 2. Bootstrap store (TDD — tests first)

- [x] 2.1 Write failing tests in `src/store/bootstrapStore.test.ts` covering: initial status
      ...
      GitHub: #78
- [x] 2.2 Extend those tests to cover retry: `retry()` after a failure resets status to
      ...
      dropped.
      GitHub: #79
- [x] 2.3 Add a test asserting `initialize()` is a no-op while an attempt is already in
      flight — two concurrent calls result in exactly one `initDatabase()` call.
      GitHub: #78
- [x] 2.4 Implement `src/store/bootstrapStore.ts` (zustand `create`, `status`, `error`,
      `initialize`, `retry`) as the minimum needed to make 2.1–2.3 pass, and keep the
      existing `console.error` logging on failure.
      GitHub: #78
- [x] 2.5 Run `npm test` and confirm the new suite passes with no regressions in the
      existing 18 suites.
      GitHub: #78

## 3. Branded splash component

- [x] 3.1 Create `src/components/AppSplash.tsx`: the logo from
      ...
      GitHub: #78
- [x] 3.2 Have `AppSplash` render the `StatusBar` with the same light/dark logic the current
      loading branch uses, so the status bar does not flip during the handoff.
      GitHub: #78
- [x] 3.3 Add a `common.retry` key to `src/i18n/locales/en.ts`, `es.ts`, `fr.ts` and `de.ts`;
      leave `common.initializingDatabase` and `common.databaseError` unchanged.
      GitHub: #79

## 4. Wire the app entry

- [x] 4.1 In `src/App.tsx`, call `SplashScreen.preventAutoHideAsync()` once at module scope
      and swallow its rejection so it cannot surface as an unhandled rejection.
      GitHub: #78
- [x] 4.2 Replace the local `useState` boot state in `App` with `useBootstrapStore`, calling
      ...
      GitHub: #78
- [x] 4.3 Rewrite the `loading` branch of `AppShell` to render `AppSplash` with the existing
      `common.initializingDatabase` text.
      GitHub: #78
- [x] 4.4 Rewrite the `error` branch to render `AppSplash` with the existing
      `common.databaseError` message plus a Paper `Button` calling `retry()`, labelled with
      `common.retry`.
      GitHub: #79
- [x] 4.5 Call `SplashScreen.hideAsync()` from the root view's `onLayout` once status is no
      ...
      GitHub: #78
- [x] 4.6 Confirm the provider order (`GestureHandlerRootView > LocaleProvider >
...
- [x] 5.1 Run `npm run typecheck`, `npm run lint` and `npm test` — all must pass.
      GitHub: #78
- [x] 5.2 Run `npx expo export --platform android` and confirm it succeeds with the plugin
...
- [x] 6.1 Note in `README.md` that the splash artwork is `assets/splash-icon.png` and that
